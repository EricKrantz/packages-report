...
lapply(got_chars, function(x) x[["playedBy"]])
lapply(got_chars, function(x) x[[18]])


## Use string and position shortcuts to extract playedBy for all characters.
map(got_chars[1:19], "playedBy")
...
lapply(got_chars, function(x) x[["id"]])
lapply(got_chars, function(x) x[[2]])
...
map_dfr(got_chars, `[`, c("name", map(got_chars, "titles")))
...
aa <- map_chr(got_chars, ~ glue_data(.x, "{name} was born {born}."))
